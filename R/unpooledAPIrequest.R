...
unpooledAPIrequest <- function(keyToUse,
  ...
  appName <- tryCatch(.apiRequesterData$appName,
    error = function(e) {
      return("An Uninitialized TornAPIRequester Project")
    }
  )
  ...
  requestString <- paste0(
    "https://api.torn.com/", section, "/", IDtoSend,
  ...
  if (identical(names(requestResult), "error")) {
    errorMessage <- paste0(
      "Error ", requestResult$error$code, ": ",
      requestResult$error$error, "."
    )
    # Errors with the key will always throw, no matter what:
    if (requestResult$error$code %in% c(
      1, # 1 => Key is empty : Private key is empty in current request
    ...
    if (requestResult$error$code %in% c(
      8, # 8 => IP block : Current IP is banned for a small period of time because of abuse.
